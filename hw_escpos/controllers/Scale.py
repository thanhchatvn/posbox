        ...
        with rs232_lock:
            try:
                ...
                for device in devices:
                    driver = rs232_devices.get(device)
                    ...
                    for protocol in SCALE_PROTOCOLS:
                        ...
                        if status:
                            ...
                            rs232_devices[path] = DRIVER_NAME
                            return connection
...
if serial:
    scale_thread = Scale()
    proxy.proxy_drivers[DRIVER_NAME] = scale_thread
...
class ScaleDriver(proxy.ProxyController):
    ...
    def get_scale_zero(self):
        if scale_thread:
            scale_thread.set_zero()
        return True

    @http.route('/hw_proxy/get_scale_tare/', type='json', auth='none', cors='*')
    def get_scale_tare(self):
        if scale_thread:
            scale_thread.set_tare()
        return True

    @http.route('/hw_proxy/get_scale_clear_tare/', type='json', auth='none', cors='*')
    def get_scale_clear_tare(self):
        if scale_thread:
